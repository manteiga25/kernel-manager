...
class io_manager:
    ...
    def set_adv_sched_param(self, device, sched, folder, val_idx):
        try:
            with open(f"/sys/block/{device}/queue/{sched}/{folder}", "w") as fd:
                fd.write(str(val_idx))
        except:
            print(val_idx, "err")

    def rende_deadline_adv_sched(self, device):
        ctk.CTkLabel(self.win_sched_adv, text=f"Deadline Advanced Sched for {device}").grid(row=0, column=1, columnspan=2, padx=5, pady=5)
    ...
    def rende_cfq_adv_sched(self, device):
        ctk.CTkLabel(self.win_sched_adv, text=f"CFQ Advanced Sched for {device}").grid(row=0, column=1, columnspan=2, padx=5, pady=5)
    ...
    def rende_adv_sched_generic(self, device, sched):
        ctk.CTkLabel(self.win_sched_adv, text=f"{sched} Advanced Sched for {device} - Generic mode").grid(row=0, column=1, columnspan=2, padx=5, pady=5)
        try:
            folders = listdir(f"/sys/block/{device}/queue/{sched}")
        ...
        for folder in range(num_folders):
            ...
            list_sched_params[folder] = ctk.CTkEntry(self.win_sched_adv, placeholder_text=list_sched_values[folder])
            list_sched_params[folder].grid(row=folder+1, column=1, padx=5, pady=5)
            list_sched_params_button[folder] = ctk.CTkButton(self.win_sched_adv, command=lambda dev=device, idx=folder: self.set_adv_sched_param(dev, sched, folders[idx], list_sched_params[idx].get()))
            list_sched_params_button[folder].grid(row=folder+1, column=2, padx=5, pady=5)
    ...
    def get_special_io_devices(self):
        try:
            devices_dr = listdir("/sys/block/")
            devices = [device for device in devices_dr if path.isdir(f"/sys/block/{device}") and device.find("zram") and device.find("sd")] # permita todos os blocos menos o zram
            return devices
        except:
            return "err"
    ...
    def rende_io_devices(self):
        ...
        for device_idx, device in zip(range(devices_len), devices):
            device_frame = LabelFrame(self.frame_global_io, text=device, background='#212121', foreground="white", labelanchor="n")
            device_frame.grid(row=device_idx // 2, column=device_idx % 2, padx=5, pady=5)
        
            # scheduler
            ctk.CTkLabel(device_frame, text="scheduler:").grid(row=0, column=0, padx=5, pady=5)
            scheduler_box[device_idx] = ctk.CTkComboBox(device_frame, state="readonly", values=self.get_device_schedulers(device), command=lambda dev=device, idx=device_idx: self.set_device_scheduler(dev, scheduler_box[idx].get()))
           ...
           # ctk.CTkButton(device_frame, text="Aplicar alteração", command=lambda idx=device_idx, dev=device: self.set_device_scheduler(dev, scheduler_box[idx].get())).grid(row=0, column=2)
            ctk.CTkButton(device_frame, text="Advanced scheduler parameters", command=lambda idx=device_idx, dev=device: self.rende_advanced_sched_params(dev, scheduler_box[idx].get())).grid(row=0, column=2, padx=5, pady=5)
       ...
       #         ctk.CTkLabel(device_frame, text=label).grid(row=row+1, column=0, padx=5, pady=5)
        #        list_io_boelans[row] = ctk.CTkSwitch(device_frame, text="off/on", command=lambda idx2=row * devices_len, dev=device, fd=folder: self.set_io_state(dev, fd, list_io_boelans[idx2].get(), idx2))
         ...
         #   """""""
            ...
            # io stat
            ctk.CTkLabel(device_frame, text="io statistics:").grid(row=1, column=0, padx=5, pady=5)
            list_io_stat[device_idx] = ctk.CTkSwitch(device_frame, text="off/on", command=lambda idx=device_idx, dev=device: self.set_io_state(dev, "iostats", list_io_stat[idx].get()))
            ...
            # random io
            ctk.CTkLabel(device_frame, text="randomize io:").grid(row=2, column=0, padx=5, pady=5)
            list_random_io[device_idx] = ctk.CTkSwitch(device_frame, text="off/on", command=lambda idx=device_idx, dev=device: self.set_io_state(dev, "add_random", list_random_io[idx].get()))
            ...
            # rotation device
            ctk.CTkLabel(device_frame, text="rotational device:").grid(row=3, column=0, padx=5, pady=5)
            list_rotational_io[device_idx] = ctk.CTkSwitch(device_frame, text="off/on", command=lambda idx=device_idx, dev=device: self.set_io_state(dev, "rotational", list_rotational_io[idx].get()))
            ...
            ctk.CTkLabel(device_frame, text="nomerges:").grid(row=4, column=0, padx=5, pady=5)
            list_nomerges_io[device_idx] = ctk.CTkComboBox(device_frame, values=nomerge_value, state="readonly")
        ...
        #    if self.get_io_state(device, "nomerges") == "1":
            ...
            ctk.CTkButton(device_frame, text="Aplicar alteração", command=lambda idx=device_idx, dev=device: self.set_io_state(dev, "nomerges", list_nomerges_io[idx].get()[0])).grid(row=4, column=2, padx=5, pady=5)

            # rq affinity
            re_affinity_value = ["0", "1", "2"] # por mudar
            ctk.CTkLabel(device_frame, text="rq affinity:").grid(row=5, column=0, padx=5, pady=5)
            list_rq_affinity_io[device_idx] = ctk.CTkComboBox(device_frame, values=re_affinity_value, state="readonly")
          #  list_nomerges_io[device_idx] = ctk.CTkComboBox(device_frame, values=nomerge_value, command=lambda idx=device_idx, dev=device: print(idx[0]))
        #    if self.get_io_state(device, "nomerges") == "1":
            ...
            ctk.CTkButton(device_frame, text="Aplicar alteração", command=lambda idx=device_idx, dev=device: self.set_io_state(dev, "rq_affinity", list_rq_affinity_io[idx].get()[0])).grid(row=5, column=2, padx=5, pady=5)
            ...
            ctk.CTkLabel(device_frame, text="max sector size:").grid(row=6, column=0, padx=5, pady=5)
            self.number_sector_selected[device_idx] = ctk.CTkLabel(device_frame, text=self.get_io_state(device, "max_hw_sectors_kb") + " KB")
            list_max_sector_io[device_idx] = ctk.CTkSlider(device_frame, from_=1, to=int(self.get_io_state(device, "max_hw_sectors_kb")), number_of_steps=logical_blk_size, command=lambda widget=device_idx, val=device_idx, dev=device: treat_read_block_val(val, int(widget), dev, self.number_sector_selected[device_idx]))
            ...
            ctk.CTkLabel(device_frame, text="read ahead").grid(row=7, column=0, padx=5, pady=5)
            self.number_ahead_selected[device_idx] = ctk.CTkLabel(device_frame, text=str(read_ahead_size) + " kb", justify="left")
          #  list_max_sector_io[device_idx] = ctk.CTkSlider(device_frame, from_=logical_blk_size, to=int(self.get_io_state(device, "max_hw_sectors_kb")) * 1000, number_of_steps=logical_blk_size, command=lambda idx=device_idx, dev=device: self.set_io_state(dev, "max_sectors_kb", list_max_sector_io[idx].get()))
            self.list_ahead_io[device_idx] = ctk.CTkSlider(device_frame, from_=64, to=71296, number_of_steps=64, command=lambda widget=device_idx, val=device_idx, dev=device: treat_read_ahead_val(val, int(widget), dev, self.number_ahead_selected[device_idx])) # wtf
            ...
            # nr_request
            ctk.CTkLabel(device_frame, text="Nr request:").grid(row=8, column=0, padx=5, pady=5)
            list_entry_nr_request[device_idx] = ctk.CTkEntry(device_frame, placeholder_text=self.get_io_state(device, "nr_requests"))
            list_entry_nr_request[device_idx].grid(row=8, column=1, padx=5, pady=5)
            ctk.CTkButton(device_frame, text="Aplicar alteração", command=lambda idx=device_idx, dev=device: self.set_io_state(dev, "nr_requests", list_entry_nr_request[idx].get())).grid(row=8, column=2, padx=5, pady=5)
