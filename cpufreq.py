...
def get_core_utilization(num_core):
    try:
        with open(f"/sys/devices/system/cpu/cpu{num_core}/?", "w") as gov:
            return gov.read().strip()
    except:
        return "err"
    ...
    def cpu_freq_frame(self):
        ...
        for core in range(int(self.num_cores)):
           ...
           # print(self.lista_state_core[core].get())
            ...
            self.lista_core_utilization[core] = ctk.CTkLabel(frame_core, text="Utilization: " + get_core_utilization(core) + "%")
            ...
            ctk.CTkLabel(frame_core, text="Governador: ").grid(column=0, row=4)
         ...
         #   self.lista_governadores[core].bind("<ComboboxSelected>", lambda event: self.muda_governador(event, core))
            self.lista_governadores[core].grid(column=1, row=4, padx=10)
            self.lista_ok[core].grid(column=2, row=4, padx=10)
            ctk.CTkLabel(frame_core, text="Preference: ").grid(column=0, row=5, pady=10)
            ...
            self.lista_energy_state[core].grid(column=1, row=5, padx=10, pady=10)
            self.lista_ok_preference[core].grid(column=2, row=5, padx=10, pady=10)
