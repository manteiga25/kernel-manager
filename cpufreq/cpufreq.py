...
class cpu_freq:
    ...
    #    cores_freq_table = self.get_cores_freq_table(num_cores)
        ...
        #    ctk.CTkLabel(frame_core, text=f"Core Type: {self.core_purpose[core]}").grid(row=2, column=0 ,padx=5, pady=5, columnspan=2, sticky="w")
         ...
         #       if self.b_cores > 0:
          #          self.b_cores -= 1
           ...
           # print(self.lista_state_core[core].get())
            ...
            self.lista_core_utilization[core].grid(column=0, row=3, columnspan=2, padx=5, sticky="w")
    ...
    def cpu_driver_frame(self):
        if path.exists("/sys/devices/system/cpu/intel_pstate/"):
            from cpufreq.pstate import intel_pstate
            intel_pstate.rende_cpu_driver_intel(self.frame_cpu_freq, self.messager)
    ...
    def cpu_eficiency_frame(self):
        ...
        from cpufreq.worqueue import cpu_power_eficient as worqueue
  ...
  #      try:
   #         with open("big_little.txt", "r") as fd:
    ...
    def create_table_clusters(self):
        self.core_purpose = []
        for core_purpose_name in self.purpose_list.keys():
            for _ in range(self.purpose_list[core_purpose_name]):
                self.core_purpose.append(core_purpose_name)
    ...
    def rende_cpu_freq(self, menu, threads, purpose_list, messager):
        ...
        self.purpose_list : dict = purpose_list
        self.create_table_clusters()
